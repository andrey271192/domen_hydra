...
Панель работает как systemd‑сервис и по кнопке «📡 Обновить…» подключается по SSH к роутерам.
...
- **Для SSH‑операций нужен `sshpass`** (установщик ставит, но если ставишь руками):
...
- Если у роутера **нестандартный SSH‑порт** — в панели во вкладке «Роутеры» укажи **`SSH порт (опц.)`**.
...
- Нестандартный SSH‑порт — не указан **«SSH порт (опц.)»**
